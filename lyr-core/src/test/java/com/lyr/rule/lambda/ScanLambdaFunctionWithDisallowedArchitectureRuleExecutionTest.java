...
class ScanLambdaFunctionWithDisallowedArchitectureRuleExecutionTest {
    ...
    static ScanLambdaFunctionWithDisallowedArchitectureRuleExecution testObject;

    @BeforeEach
    public void beforeEach() {
        mockedLambdaConnector.when(LambdaConnector::create).thenReturn(mockedLambdaConnectorInstance);
        testObject = new ScanLambdaFunctionWithDisallowedArchitectureRuleExecution();
    }
    ...
    void testThatRuleReturnsOnlyDisallowedArchitecture() {
        ...
        final var listOfFunctionConfigurations = List.of(
                createFunctionConfiguration(FUNCTION_1, Architecture.ARM64),
                createFunctionConfiguration(FUNCTION_2),
                createFunctionConfiguration(FUNCTION_3, Architecture.ARM64),
                createFunctionConfiguration(FUNCTION_4));
    ...
    private static FunctionConfiguration createFunctionConfiguration(final String functionName) {
        return createFunctionConfiguration(functionName, Architecture.X86_64);
    }

    private static FunctionConfiguration createFunctionConfiguration(
            final String functionName, final Architecture architecture) {
